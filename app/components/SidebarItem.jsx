...
export default function SidebarItem({text, alert, icon}) {
  ...
  return (
    <>
      <li className={`${toggler? "gap-3 rounded":" rounded-full"} ${darkMode? "shadow-sm hover:shadow-md shadow-black": "hover:bg-violet-700 shadow-sm hover:shadow-2xl"} rounded-full flex relative cursor-pointer p-3  duration-300  hover:scale-110`} onClick={activeness} onMouseEnter={show} onMouseLeave={leave}>
            {icon}
