...
export default function Settings() {
  ...
  return (
    ...
    { darkMode ? <MdWbSunny className={`${settingsoOn? "rotate-180 scale-105":"rotate-0"} text-yellow-600  text-3xl lg:text-4xl cursor-pointer transition-all duration-300`} onClick={toggleSettings} /> : <WiMoonWaxingCrescent3 className={`${settingsoOn? "rotate-180 scale-105":"rotate-0"}  text-3xl lg:text-4xl cursor-pointer transition-all duration-300`} onClick={toggleSettings} />}
    ...
    <div className={`fixed p-0 z-30 top-[88%] left-[90%] sm:left-[92%]`} onClick={toggleSocialMedia}>
      <div className={`relative`}>
        <IoShareSocialSharp className={`${showing? "rotate-180 shadow-none scale-105":"rotate-0"} ${darkMode? "bg-blue-700":"bg-violet-600"} text-[30px] text-slate-950 cursor-pointer transition-all duration-300 p-2 absolute z-30 rounded-full`}/>
